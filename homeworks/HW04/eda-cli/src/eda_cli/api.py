...
class QualityFlags(BaseModel): # модель ответа дз
    flags: dict[str, Any] | None = Field(
        default=None,
        description="Флаги с подробностями",
    )
...
# ---------- домашнее задание: /quality-flags-from-csv ---------- 

@app.post(
    "/quality-flags-from-csv",
    response_model=QualityFlags,
    tags=["quality"],
    summary="Возвращает полный набор флагов качества по CSV-файлу",
)
async def quality_flags_from_csv(file: UploadFile = File(...)) -> QualityFlags:
    """
    Принимает CSV-файл, запускает summarize_dataset + missing_table + compute_quality_flags
    и возвращает ВСЕ флаги качества (bool), включая те, что были реализованы в HW03.
    """
    ...
    summary = summarize_dataset(df)
    missing_df = missing_table(df)
    all_flags = compute_quality_flags(summary, missing_df)

    flags = {k: bool(v) for k, v in all_flags.items() if isinstance(v,bool)}
    return QualityFlags(flags=flags)